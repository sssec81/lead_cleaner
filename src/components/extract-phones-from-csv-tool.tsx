...
async function handleFileUpload(event: React.ChangeEvent<HTMLInputElement>) {
 ...
 ? readLocalStorage("leadcleanr:extract-phone-csv:preferred-column")
 : null;
          ...
          {extracted.results.length ? (
            <div className="lc-table-scroll">
              <table aria-label="Extracted phone preview" className="lc-data-table lc-data-table-compact">
                <caption className="sr-only">Valid phone numbers extracted from the selected CSV column.</caption>
                <thead>
                  <tr>
                ...
                <tbody>
                  {extracted.results.slice(0, PREVIEW_LIMIT).map((phone, idx) => (
                    <tr key={phone}>
                      ...
                      <td className="lc-data-table-value" title={phone}>{phone}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
