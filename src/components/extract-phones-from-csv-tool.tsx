...
async function handleFileUpload(event: React.ChangeEvent<HTMLInputElement>) {
 ...
 (detection) => detection.header === selectedColumn,
    ...
    <CsvWorkspaceShell
      ...
      emptyStateSubtitle="Upload your CSV to isolate and export phone numbers instantly. Processed locally."
      ...
      toolbar={
        <div className="flex-1 min-w-[200px]">
          <label htmlFor="phone-column-select" className="block text-[11px] font-bold uppercase tracking-widest text-slate-500 mb-2">
            Target Column
          </label>
          <select
      ...
      summary={
        <div className="flex min-w-full flex-nowrap items-stretch">
          ...
          <div className="flex-1 bg-indigo-50/30 p-5 sm:px-6 transition-colors hover:bg-indigo-50/60 relative overflow-hidden min-w-[160px]">
            ...
            <p className="text-[10px] font-bold uppercase tracking-widest text-indigo-600">Clean Phones</p>
            ...
            <table className="min-w-full text-left text-sm whitespace-nowrap border-collapse">
              <thead className="sticky top-0 z-10 bg-slate-50 shadow-[0_1px_0_0_#e2e8f0]">
                <tr>
                  ...
                  <th className="px-5 py-3.5 font-bold text-slate-900">PHONE</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100/60">
                {extracted.results.slice(0, PREVIEW_LIMIT).map((phone, idx) => (
                  <tr key={phone} className="hover:bg-slate-50/80 transition-colors group">
                    ...
                    <td className="px-5 py-3 text-slate-700 max-w-[280px] truncate" title={phone}>{phone}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="flex w-full flex-col items-center justify-between gap-6 sm:flex-row">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500 mt-0.5">{extracted.summary.cleanPhonesReady.toLocaleString()} valid phones found.</p>
              </div>
            </div>
