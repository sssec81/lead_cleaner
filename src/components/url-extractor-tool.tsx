...
import { Check, Clipboard, Download, FileText, Link as LinkIcon } from "lucide-react";
...
export function UrlExtractorTool() {
  ...
  const { results, stats } = extractUrlsFromText(input);
        ...
        <div className="mb-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm leading-6 text-[color:var(--muted)]">
              Paste messy text, pull out website links, normalize the format,
              remove duplicates, and export the clean list.
            </p>
          </div>
        </div>

        <label htmlFor="url-raw-text" className="mb-2 block text-sm font-semibold">
          Paste your text
        </label>
        <textarea
          id="url-raw-text"
      ...
      <div className="space-y-6">
        <div className="rounded-[2rem] border border-[color:var(--line)] bg-[color:var(--surface-strong)] p-5 shadow-[var(--shadow)] sm:p-7">
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2">
            <StatCard label="URLs found" value={stats.totalFound} />
            ...
            <StatCard label="Clean URLs ready" value={stats.cleanResults} accent />
          </div>
        </div>
          ...
          <div className="mt-4 min-h-[22rem] rounded-[1.5rem] border border-dashed border-[color:var(--line)] bg-white/70 p-4">
            {results.length ? (
              ...
              <p className="text-sm leading-7 text-[color:var(--muted)]">
                No URLs detected yet. Paste text with links to generate a clean
                list.
              </p>
            )}
          </div>
