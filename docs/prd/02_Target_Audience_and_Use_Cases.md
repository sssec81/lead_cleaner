...
A user is manually researching prospects on LinkedIn and pasting snippets one by one. By turning on batch mode, they can keep pasting new snippets onto new lines, and the workspace incrementally parses and appends valid matches (like URLs or phone numbers) without losing the previous extractions.
